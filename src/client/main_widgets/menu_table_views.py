...
class MenuTableViews(QtWidgets.QFrame):
    ...
    def fill_table(self, database_model: BaseModel) -> None:
        self.clear_table()
        if 'id' in database_model._meta.sorted_field_names:
            database_model._meta.sorted_field_names.remove('id')
        ...
        for model in database_model.select():
            for j, column, field in zip([i for i in range(len(database_model._meta.sorted_field_names))], database_model._meta.sorted_field_names, [getattr(database_model, field) for field in database_model._meta.sorted_field_names]):
                if column == 'id':
                    continue
        ...
        fields = [field.replace('_id', '') for field in database_model._meta.sorted_field_names]
    ...
    def create_data(self, dialog: TableAction) -> str:
        ...
        for i in range(0, layout.count()):
            item = layout.itemAt(i)
            widget: QtWidgets.QLineEdit | QtWidgets.QComboBox | QtWidgets.QDateTimeEdit = item.widget()
            if isinstance(widget, QtWidgets.QLineEdit):
                ...
                data[widget.objectName()] = widget.dateTime().toString('dd.MM.yyyy hh:mm')
        return data
    ...
    def upd_button_clicked(self) -> None:
        ...
        for index, column in zip(range(0, self.table.columnCount()), self.database_model._meta.sorted_field_names):
            column_data = self.table.model().index(row_id, index).data()
        ...
        result = upd_action(row_id, self.database_model, data)
